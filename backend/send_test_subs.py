...
# --- BATCH 2: bills / insurance / warranty / bank transactions — ALREADY SENT.
#     Commented so a re-run won't duplicate them. Uncomment to resend from scratch.
# _SENT_BATCH2 = [
...
#      f"Your electricity bill of Rs.1,286 is due on {d(13)}. Account: 4567890123.\n\nBESCOM"),
#     ("Dell", "orders@dell.com",
#      "Invoice & warranty - Dell XPS 13",
#      "Thank you for your purchase.\n\nDell XPS 13 (Rs.1,24,990). Premium Support "
#      f"warranty valid until {d(165)}.\n\nDell"),
# ]
# The HDFC UPI transaction alerts (Swiggy x5 + misc x7) were also part of batch 2 —
# see git history if you need to resend them.

# --- BATCH 1 (RESEND): subscriptions. The first send showed as work.faltumail@gmail.com
#     because Gmail strips the friendly sender name when the display embeds a strict-DMARC
#     brand domain (netflix.com, spotify.com, google.com, amazon.in, audible.in). Using
#     off-brand sender domains keeps the "Name (address)" display Gmail shows for
#     Dell/BESCOM/ICICI. Addresses are cosmetic (Gmail rewrites the real From to the
#     authenticated account); the body is what extraction reads, so it is unchanged.
MESSAGES = [
    (
        "Netflix", "billing@netflix-in.email",
    ...
    (
        "Audible", "no-reply@audible-in.email",
    ...
    (
        "Amazon Prime", "auto-confirm@amazon-in.email",
